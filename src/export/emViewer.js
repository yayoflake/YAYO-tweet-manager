    ...
    init: function (ctx) {
        ...
        function buildNodeForItem(item, index) {
            ...
            if (item.type === 'tweet') {
                ...
                muteBtn.style.cssText = "position: absolute; top: 8px; right: 8px; background: none; border: none; padding: 4px; cursor: pointer; z-index: 10; display: flex; align-items: center; justify-content: center; transition: color 0.2s;";
                
                // 뮤트 SVG 아이콘 (Speaker Off)
                const muteSvg = `<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z"/></svg>`;
                muteBtn.innerHTML = muteSvg;
                
                const isMuted = _em.mutedIds.has(item.tweetId);
                const updateMuteStyle = (muted) => {
                    card.style.opacity = muted ? "0.35" : "1";
                    card.style.filter = muted ? "grayscale(0.5)" : "none";
                    muteBtn.style.color = muted ? "#1a73e8" : "#ccc"; // 활성화 시 파란색, 비활성화 시 연한 회색
                    muteBtn.title = muted ? "내보내기에 포함 (뮤트 해제)" : "내보내기에서 제외 (뮤트)";
                };

                card.style.position = "relative";
                updateMuteStyle(isMuted);

                muteBtn.addEventListener("click", (e) => {
                    e.stopPropagation();
                    const nowMuted = !_em.mutedIds.has(item.tweetId);
                    if (nowMuted) _em.mutedIds.add(item.tweetId);
                    else _em.mutedIds.delete(item.tweetId);
                    
                    updateMuteStyle(nowMuted);
                ...
                muteBtn.addEventListener("mouseenter", () => {
                    if (!_em.mutedIds.has(item.tweetId)) muteBtn.style.color = "#666";
                });
                muteBtn.addEventListener("mouseleave", () => {
                    if (!_em.mutedIds.has(item.tweetId)) muteBtn.style.color = "#ccc";
                });
